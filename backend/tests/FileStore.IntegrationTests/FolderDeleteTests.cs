...
public class FolderDeleteTests(IntegrationTestFixture fixture)
{
    ...
    private static async Task<List<FolderDto>> ListFoldersAsync(HttpClient client) =>
        (await client.GetFromJsonAsync<List<FolderDto>>("/v1/folders?all=true"))!;

    private static async Task<List<TrashItemDto>> ListTrashAsync(HttpClient client) =>
        (await client.GetFromJsonAsync<List<TrashItemDto>>("/v1/trash"))!;
    ...
    public async Task Delete_CarpetaVacia_SinRecursive_Funciona()
    {
        ...
        var response = await client.DeleteAsync($"/v1/folders/{folder.Id}");
    ...
    public async Task Delete_ConSubcarpetas_SinRecursive_Devuelve409()
    {
        ...
        var response = await client.DeleteAsync($"/v1/folders/{padre.Id}");
        Assert.Equal(HttpStatusCode.Conflict, response.StatusCode);
    }
    ...
    public async Task Delete_Recursive_BorraElSubarbolYMandaLosArchivosALaPapelera()
    {
        ...
        var response = await client.DeleteAsync($"/v1/folders/{docs.Id}?recursive=true");
        Assert.Equal(HttpStatusCode.NoContent, response.StatusCode);
        ...
        var restore = await client.PostAsync($"/v1/trash/{raiz.Id}/restore", null);
    ...
    public async Task Delete_Recursive_LosArchivosSiguenOcupandoCuota()
    {
        ...
        var antes = await client.GetFromJsonAsync<UsageResponse>("/v1/me/usage");

        await client.DeleteAsync($"/v1/folders/{folder.Id}?recursive=true");
        ...
        var despues = await client.GetFromJsonAsync<UsageResponse>("/v1/me/usage");
    ...
    public async Task Delete_Recursive_ConservaLaFechaDeBorradoDeLosQueYaEstabanEnLaPapelera()
    {
        ...
        await client.DeleteAsync($"/v1/files/{yaBorrado.Id}");
    ...
    public async Task Delete_CarpetaDeOtroCliente_Devuelve404()
    {
        ...
        var response = await clientB.DeleteAsync($"/v1/folders/{carpetaDeA.Id}?recursive=true");
